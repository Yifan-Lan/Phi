...
class SDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        ad_train_dataset: Optional[Dataset] = None,
        ad_eval_dataset: Optional[Union[Dataset, Dict[str, Dataset]]] = None,
        ...
        with PartialState().local_main_process_first():
            ...
            ad_train_dataset = ad_train_dataset.map(self.tokenize_row, num_proc=self.dataset_num_proc, writer_batch_size=10)
            if ad_eval_dataset is not None:
                ad_eval_dataset = ad_eval_dataset.map(
                    self.tokenize_row, num_proc=self.dataset_num_proc, writer_batch_size=10
                )
        ...
        self.ad_train_dataset = ad_train_dataset
        self.ad_eval_dataset = ad_eval_dataset
    ...
    def get_train_dataloader(self, processed_images=None) -> DataLoader:
        """
        ...
        if self.precompute_ref_log_probs and not self._precomputed_train_ref_log_probs:
            ...
            if self.multi_images == True:
                data_loader = self.accelerator.prepare(DataLoader(self.train_dataset, **dataloader_params))
                for image_id in range(len(processed_images)):
                    image = processed_images[image_id]
                    reference_chosen_logps = []
                    reference_rejected_logps = []
                    for padded_batch in tqdm(iterable=data_loader, desc=f"Train dataset reference log probs {image_id}"):
                        padded_batch["prompt_pixel_values"] = image.repeat(padded_batch["prompt_input_ids"].shape[0],1,1,1)
                    ...
                    self.train_dataset = self.train_dataset.add_column(
                        name=f"reference_chosen_logps{image_id}", column=all_reference_chosen_logps
                    )
                    self.train_dataset = self.train_dataset.add_column(
                        name=f"reference_rejected_logps{image_id}", column=all_reference_rejected_logps
                    )
                
                #self.train_dataset = self.ad_train_dataset
                del self.ad_train_dataset
        ...
        self.train_dataloader = super().get_train_dataloader()
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if self.precompute_ref_log_probs and not self._precomputed_eval_ref_log_probs:
            ...
            eval_dataset = self.ad_eval_dataset
            del self.ad_eval_dataset
            torch.cuda.empty_cache()
        ...
        self.eval_dataloader = super().get_eval_dataloader(eval_dataset=eval_dataset)
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid" or self.loss_type == "cedpo":
            losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing
            )
        elif self.loss_type == "ce":
            losses = -policy_chosen_logps  # Actually we use NLL loss directly line 1473 
    ...
    def concatenated_forward(
        ...
        concatenated_batch = self.accelerator.prepare(concatenated_batch)
        ...
        if self.is_vision_model:
            if "pixel_values" in concatenated_batch:
                model_kwargs["pixel_values"] = concatenated_batch["pixel_values"].cuda()
            if "pixel_attention_mask" in concatenated_batch:
                model_kwargs["pixel_attention_mask"] = concatenated_batch["pixel_attention_mask"].cuda()
    ...
    def get_batch_loss_metrics(
        ...
        if image_id is not None:
            if (
                f"reference_chosen_logps{image_id}" in batch
                and f"reference_rejected_logps{image_id}" in batch
            ):
                reference_chosen_logps = torch.tensor(batch[f"reference_chosen_logps{image_id}"])
                reference_rejected_logps = torch.tensor(batch[f"reference_rejected_logps{image_id}"])
            else:
                with torch.no_grad():
                    if self.ref_model is None:
                        with self.null_ref_context():
        ...
        if self.loss_type == "cedpo":
            return losses.mean() + self.beta_ce*policy_nll_loss.mean(), metrics
    ...
    def compute_loss(
        ...
        #inputs["images"].requires_grad = True
        with compute_loss_context_manager:
            loss, metrics = self.get_batch_loss_metrics(model, inputs, train_eval="train", image_id=image_id)
