...
# Custom arguments for command line input
@dataclass
class CustomPatchArguments:
    """Custom arguments for the patch training script."""
    ...
    patch_size: int = field(
        default=168,
        metadata={"help": "Patch size for the adversarial patch"}
    )
...
def parse_custom_args():
    ...
    parser.add_argument('--patch_size', type=int, default=168,
                       help='Patch size for the adversarial patch')
...
if __name__ == "__main__":
    ...
    patch_size = custom_args.patch_size if custom_args.patch_size else 168
    ...
    model_kwargs = dict(
        ...
        revision='a272c74',  # use the specific revision
    ...
    # Dataset
    ################
    # Calculate project root directory for data paths
    ...
    patch = torch.full((1, 3, patch_size, patch_size), 0.5).cuda()
    ...
    total_patch_grad = torch.zeros_like(patch)
    for i in range(num_iter):
        ...
        for j in range(batch["prompt_pixel_values"].shape[0]):
            ...
            adv_patch = patch.clone()
            adv_image = x.clone().cuda()
            adv_image[:, :, :patch_size, :patch_size] = adv_patch
        ...
        patch_grad = batch["prompt_pixel_values"].grad.sum(dim=0, keepdim=True)[:, :, :patch_size, :patch_size].detach().cuda()
        total_patch_grad += patch_grad
        ...
        if (i+1) % training_args.gradient_accumulation_steps == 0:
            grad = total_patch_grad.detach()
            patch.data = (patch.data - alpha * grad.sign()).clamp(0, 1)
        ...
        if (i + 1) % 1000 == 0:
            with torch.no_grad():
                ...
                output = trainer.model.generate(pixel_values=eval_batch["prompt_pixel_values"], input_ids=eval_batch["prompt_input_ids"], attention_mask=eval_batch["prompt_attention_mask"], max_new_tokens=512, do_sample=True,
                    pad_token_id=trainer.tokenizer.tokenizer.pad_token_id)
                print("Response With Phi Image:")
                print(processor.decode(output[0]))
                output = trainer.model.generate(pixel_values=norm(x).repeat(eval_batch["prompt_input_ids"].shape[0],1,1,1), input_ids=eval_batch["prompt_input_ids"], attention_mask=eval_batch["prompt_attention_mask"], max_new_tokens=512, do_sample=True,
                    pad_token_id=trainer.tokenizer.tokenizer.pad_token_id)
                print("Response With Clean Image:")
                print(processor.decode(output[0]))
        if (i + 1) % 100 == 0:        
            file_name = f'patch_iter{i+1}.bmp'
            save_path = os.path.join(save_dir, file_name)
            save_image(patch, save_path)
            print(f"Saved adversarial patch at iteration {i + 1} to {save_path}")
